...
TUNNEL\_TOKEN=CopiedToken

Create duplicati.env file with the following lines:
SETTINGS\_ENCRYPTION\_KEY=whateverseemsgood
DUPLICATI\_\_WEBSERVICE\_PASSWORD=Yourpassword
...
you may then proceed to login by the password set by the DUPLICATI\_\_WEBSERVICE\_PASSWORD
...
Keep in mind if you set up duplicati on new device your SETTINGS\_ENCRYPTION\_KEY needs to be the same as the previous device to access previously made backups, you can use the configuration file from earlier to make the process seemless since docker containers are standardised.
